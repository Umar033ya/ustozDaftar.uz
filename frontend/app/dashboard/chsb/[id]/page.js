...
export default function ChSBResult() {
  ...
  const handleScoreChange = (studentIndex, value) => {
    ...
    if (value !== "" && (isNaN(score) || score < 0 || score > assessment.maxScore)) {
      return;
    }
  ...
  const handleExport = () => {
    exportChSBToExcel(assessment, results, stats, schoolName);
  };
  ...
  const calculateStatistics = () => {
    ...
    const totals = (validResults.length > 0 ? validResults : results).map((r) => (r.total === "" ? 0 : r.total || 0));
    ...
    <DashboardLayout>
      ...
      <div className="mb-4 sm:mb-6 print:hidden">
        <Link href="/dashboard/chsb" className="text-blue-600 hover:text-blue-700 text-sm font-medium mb-2 inline-block">
          ← ChSB ro'yxatiga qaytish
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
              ChSB {assessment.number} - {assessment.subject}
            </h1>
      ...
      <div className="bg-white border border-gray-300 shadow-sm rounded-lg mb-6 p-4 sm:p-6 print:border-none print:shadow-none print:p-0 print:mb-0">
        ...
        <div className="mb-4 sm:mb-6 text-center border-b-2 border-gray-900 pb-3">
          <h2 className="text-base sm:text-lg md:text-xl font-extrabold text-gray-900 tracking-tight leading-snug">
            {schoolName || "Maktab"}ning {assessment.className}-sinf o'quvchilarining {assessment.subject} fanidan {assessment.academicYear}-o'quv yili {assessment.number}-ChSB natijalari ({assessment.date})
          </h2>
        </div>
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-6 print:hidden">
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
              ...
              <p className="text-lg font-bold text-gray-900 mt-0.5">{stats.maxScore}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
        ...
        <div className="print:hidden mb-6">
          <StudentPercentageChart results={results} title="O'quvchilar ChSB ko'rsatkichi (%)" />
        </div>
          ...
          <table className="w-full border-collapse text-xs sm:text-sm">
            <thead className="bg-gray-100 text-gray-900 font-bold border-b-2 border-gray-400">
              <tr>
                ...
                <th className="px-3 py-2 border border-gray-300 text-left min-w-[180px] sm:min-w-[240px]">
                  F.I.Sh
                </th>
                <th className="px-3 py-2 border border-gray-300 text-center w-28 bg-gray-200">
                  Jami ball
                </th>
                <th className="px-3 py-2 border border-gray-300 text-center w-24 bg-gray-200">
                  %
                </th>
              </tr>
            </thead>
                ...
                <tr key={result.studentId} className="hover:bg-gray-50/80 transition-colors">
                  ...
                  <td className="p-1 border border-gray-300 text-center">
                    <input
                      ...
                      value={result.total === "" || result.total === undefined ? "" : result.total}
                      onChange={(e) => handleScoreChange(studentIndex, e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                      ...
                      className="w-20 sm:w-24 h-8 px-1 text-center font-semibold border border-gray-300 rounded-md focus:outline-hidden focus:ring-2 focus:ring-blue-500 bg-white mx-auto"
                      placeholder="0"
                    />
                  </td>
            ...
            {stats && (
              <tfoot className="bg-gray-100 font-bold border-t-2 border-gray-400 text-gray-900">
                <tr>
                  ...
                  <td className="px-3 py-2.5 border border-gray-300 text-center font-extrabold text-blue-700 bg-gray-200">
                    {stats.averageScore}
                  </td>

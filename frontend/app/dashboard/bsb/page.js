...
export default function BSBList() {
  ...
  useEffect(() => {
    ...
    if (assessmentsJson) {
      ...
      setAssessments(bsbAssessments.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
    }
  }, []);
  ...
  const handleDeleteAssessment = (id, e) => {
    ...
    if (confirm("Ushbu BSB baholash natijalarini o'chirmoqchimisiz?")) {
      ...
      if (assessmentsJson) {
        ...
        setAssessments(updated.filter((a) => a.type === "BSB"));
      }
    }
  };
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {assessments.map((assessment) => {
                  const validResults = assessment.results.filter((r) => r.total > 0);
